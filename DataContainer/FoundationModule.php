...
class FoundationModule extends Contao_BE
{
	...
	{
		return Controller::getTemplateGroup('navleft_');
	}
	...
	{
		return Controller::getTemplateGroup('navright_');
	}
	...
	{
		$arrModules = array();
		$objModules = Database::getInstance()->prepare("SELECT id, name FROM tl_module WHERE pid=(SELECT pid FROM tl_module WHERE id=?) AND id!=? ORDER BY name")->execute(Input::get('id'),Input::get('id'));
	...
	*/
	...
	{
		$objTypes = Database::getInstance()->prepare("SELECT sc_type FROM tl_module WHERE id=?")->execute(Input::get('id'));
