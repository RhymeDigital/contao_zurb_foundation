...
namespace Rhyme\ContaoZurbFoundationBundle\DataContainer;
...
class FoundationContent extends Contao_BE
{
	 ...
	 * @return string
	 */
    public function setTabTitle($varValue)
    {
        if(empty($varValue))
        {
    ...
    public function getArticles()
    {
		...
		if ($objArticle->numRows)
		{
			...
			$arrRoot = Database::getInstance()->getChildRecords($objPage->rootId, 'tl_page');
		...
		{
			$objArticle = Database::getInstance()->execute("SELECT a.id, a.pid, a.title, a.inColumn, p.title AS parent FROM tl_article a LEFT JOIN tl_page p ON p.id=a.pid" . (!empty($arrRoot) ? " WHERE a.pid IN(". implode(',', array_map('intval', array_unique($arrRoot))) .")" : "") . " ORDER BY parent, a.sorting");
		}
		else
		{
			...
			$objArticle = Database::getInstance()->execute("SELECT a.id, a.pid, a.title, a.inColumn, p.title AS parent FROM tl_article a LEFT JOIN tl_page p ON p.id=a.pid WHERE a.pid IN(". implode(',', array_map('intval', array_unique($arrPids))) .") ORDER BY parent, a.sorting");
		}
