...
class RevealModalWindow extends Contao_Module
{
	...
	public function generate()
	{
		if (TL_MODE == 'BE')
		{
			...
			$objTemplate->wildcard = '### ' . Utf8::strtoupper($GLOBALS['TL_LANG']['FMD']['foundation_revealmodalwindow'][0]) . ' ###';
		...
		if (!empty($articles))
			foreach ($articles as $item)
			{
				if (isset($item['article']))
					$this->Template->revIncludeArticle .= Controller::getContentElement($item['article']). "\n";
			}
		...
		if (!empty($modules))
    		foreach ($modules as $item)
    		{
    			if (isset($item['module']))
    				$this->Template->revIncludeModule .= Controller::getFrontendModule($item['module']). "\n";
    		}
		...
		if ($this->useImage && $this->singleSRC != '')
		{
			...
			elseif (is_file(TL_ROOT . '/' . $objModel->path))
			{
				...
				if ($objFile->isGdImage)
				{
					...
					$src = System::getContainer()->get('contao.image.image_factory')->create(TL_ROOT . '/'. $objModel->path, $size)->getUrl(TL_ROOT);
